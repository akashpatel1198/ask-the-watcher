// Filter comic pages by byte size and write the page list for scraping.
// Reads pre-computed sizes from analyze-comics.js output.
//
// Usage: node scripts/enumerate/enumerate-comics.js <min-bytes>
//   e.g. node scripts/enumerate/enumerate-comics.js 4000

import { writeFile, readFile } from "fs/promises";

const SIZES_PATH = "scripts/enumerate/output/sizes_comics.json";
const OUT_PATH = "scripts/data/pages_comics.json";

const threshold = Number(process.argv[2]);
if (!threshold) {
  console.error("Usage: node scripts/enumerate/enumerate-comics.js <min-bytes>");
  console.error("  Run analyze-comics.js first to generate size data.");
  process.exit(1);
}
...
try {
  ...
  console.error(`No size data found at ${SIZES_PATH} — run analyze-comics.js first.`);
  process.exit(1);
}
