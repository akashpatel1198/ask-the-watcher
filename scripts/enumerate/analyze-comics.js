// Analyze comic issue pages from Category:Comics.
// Paginates the full category, batch-fetches byte sizes, prints histogram.
// Saves raw title+size data to enumerate/output/ for enumerate-comics.js to consume.
//
// Resumable — saves progress after each step. Re-run safely if interrupted.
//
// Usage: node scripts/enumerate/analyze-comics.js
//
// WARNING: ~70,000 pages — this will take a while.
// Pagination: ~141 batches × 800ms ≈ ~2 min
// Size lookups: ~1,400 batches × 800ms ≈ ~19 min
// Total: ~20–25 min
...
const TITLES_CACHE = `${OUT_DIR}/.titles_comics.json`;
const SIZES_PATH = `${OUT_DIR}/sizes_comics.json`;
const PROGRESS_PATH = `${OUT_DIR}/.progress_comics.json`;
await mkdir(OUT_DIR, { recursive: true });

// ---------------------------------------------------------------------------
// 1. Paginate Category:Comics → collect all page titles (resumable)
// ---------------------------------------------------------------------------
...
async function fetchByteSizes(titles) {
  ...
  for (let i = 0; i < titles.length; i += batchSize) {
    ...
    // Save progress every 500 batches (~25,000 titles)
    if ((i / batchSize) % 500 === 0 && i > 0) {
...
console.log("Analyzing comics from Category:Comics...\n");

const allTitles = await fetchAllCategoryMembers("Category:Comics");
...
console.log(`  node scripts/enumerate/enumerate-comics.js <min-bytes>`);
