...
/**
 * Delay between requests. Set to 0 for natural latency only.
 * Backoff still uses this as a base (min 800ms for backoff math).
 */
...
/**
 * Batch-resolve multiple image filenames to CDN URLs in a single API call.
 * MediaWiki supports up to 50 titles per request via pipe-separated values.
 *
 * @param {string[]} filenames - Array of image filenames
 * @returns {Map<string, string|null>} Map from original filename → URL (or null)
 */
export async function resolveImageUrlBatch(filenames) {
  const results = new Map();
  if (!filenames || filenames.length === 0) return results;

  // Clean filenames and build lookup map
  const cleanedMap = new Map(); // cleaned → original
  for (const f of filenames) {
    if (!f) continue;
    const cleaned = f.replace(/^\s*(?:File:|Image:)/i, "").trim();
    if (cleaned) cleanedMap.set(cleaned, f);
  }

  if (cleanedMap.size === 0) return results;

  const titles = [...cleanedMap.keys()].map((c) => `File:${c}`).join("|");
  ...
  for (const page of Object.values(pages)) {
    // page.title is "File:Cleaned_Name.jpg" — strip "File:" to match back
    const cleanedKey = page.title.replace(/^File:/i, "");
    const original = cleanedMap.get(cleanedKey);
    if (original && page.imageinfo && page.imageinfo[0]) {
      results.set(original, page.imageinfo[0].url);
    }
  }

  // Fill nulls for anything not resolved
  for (const f of filenames) {
    if (f && !results.has(f)) results.set(f, null);
  }
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (status === 429 || (status >= 500 && status < 600)) {
        const backoffMs = Math.max(800, DEFAULT_DELAY_MS) * Math.pow(2, attempt + 1);
